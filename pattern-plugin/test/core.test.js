...
test('horizontal half grid leaves a half-width gap at 100%',()=>{const s=core.normalizeSettings({...base,columns:4,rows:1,halfGrid:true,halfHorizontal:true,halfVertical:false});const f=core.fitDimensions(100,20,s,100);assert(Math.abs(f.width-(s.cellWidth*2/3))<1e-9);});
test('vertical half grid constrains height independently',()=>{const s=core.normalizeSettings({...base,columns:1,rows:4,halfGrid:true,halfHorizontal:false,halfVertical:true});const f=core.fitDimensions(20,100,s,100);assert(Math.abs(f.height-(s.cellHeight*2/3))<1e-9);});
test('half-size grid preserves its minimum gap above 100% size',()=>{const s=core.normalizeSettings({...base,columns:3,rows:3,halfGrid:true});assert.deepEqual(core.fitDimensions(100,100,s,180),core.fitDimensions(100,100,s,100));});
