...
Тот же движок работает как обои рабочего стола — по логике системных
динамических обоев macOS («Macintosh» и др.), а не как программа поверх:

- **Узор становится настоящими системными обоями.** После каждой волны
  готовый кадр записывается в файл и ставится через
  `NSWorkspace.setDesktopImageURL` — это тот же механизм, что и выбор
  обоев в System Settings. Поэтому **тот же узор виден на экране
  блокировки и в окне входа** — macOS сама показывает там текущие обои.
- Между сменами агент **ничего не рисует**: на рабочем столе лежит
  обычная статичная картинка, окон нет, нагрузка нулевая.
- **После разблокировки** (и при пробуждении от сна) на долю секунды
  появляется сцена на уровне обоев, начинающаяся с точно того же кадра
  (подмена незаметна), проигрывается ровно одна волна морфа в новый
  случайный узор — и новый кадр снова коммитится как системные обои.
...
при входе и сразу запускает. Файлы обоев хранятся в
`~/Library/Application Support/Patternique/`. Мониторы обрабатываются
каждый со своим узором.

Убрать: `make wallpaper-uninstall` (свои обои macOS верните в System
Settings → Wallpaper).
