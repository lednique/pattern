...
test(index.includes("document.addEventListener('visibilitychange'")&&index.includes('if(document.hidden){scheduleNext();return;}'),'returning to the tab restarts cleanly instead of freezing or overlapping layers');
test(index.includes('Watchdog')&&index.includes('restart();')&&index.includes('childElementCount'),'a watchdog guarantees a pattern is always visible');
...
test(index.includes("decor=Math.random()<0.12?{emoji:pick(EMOJI)}")&&index.includes("shiftX===0&&shiftY===0"),'emoji appears only as the plugin allows: an intersection symbol when offsets are 0');
test(index.includes("fig1=Math.random()<0.06?{emoji:pick(EMOJI)}")&&index.includes("Math.random()<0.12?{emoji:pick(EMOJI)}"),'emojis stay possible but appear much less often');
...
test(index.includes('backdrop-filter:blur(16px)')&&index.includes('.plan.selected { background:rgba(20,20,20,.55)'),'translucent blocks use 16px gaussian blur; plan active/inactive styles are swapped');
