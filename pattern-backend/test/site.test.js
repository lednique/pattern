...
test(index.includes('class="hero"')&&index.includes('.hero { margin-bottom:20px; background:linear-gradient(90deg, rgba(124,122,31,.62)')&&index.includes('.hero::after')&&index.includes('class="heroTop"')&&index.includes('.featureBar { display:grid; grid-template-columns:repeat(3,1fr)')&&!index.includes('.featureBar { display:flex'),'hero block uses a richer darker accent with the license-card treatment');
...
test(index.includes('border-radius:10000px; background:var(--tint-dark); backdrop-filter:blur(16px); -webkit-backdrop-filter:blur(16px); box-shadow:none')&&index.includes('.pay:disabled { background:var(--tint-dark); color:var(--muted); cursor:not-allowed; box-shadow:none; }'),'email, promo, and the disabled pay button share the inactive plan color with no shadows');
test(index.includes('.step { flex:1; min-width:150px; background:var(--tint-dark)')&&index.includes('box-shadow:var(--pill-shadow); }')&&index.includes('.step::before')&&index.includes('mix-blend-mode:plus-lighter; }')&&!index.includes('box-shadow:var(--card-inset); }'),'the pay/get key/activate steps use the inactive plan card treatment');
