...
test((index.match(/backdrop-filter:blur\(16px\)/g)||[]).length>=7,'every translucent block gets the 16px gaussian backdrop blur');
...
test(index.includes('.plan.selected { background:linear-gradient(90deg, rgba(222,221,116,.16)')&&index.includes('.plan { border:none; border-radius:36px; padding:16px 18px; cursor:pointer; background:rgba(20,20,20,.55)'),'the selected plan is the colored card; unselected plans are dark translucent');
