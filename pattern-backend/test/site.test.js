...
test(index.includes('function renderFrost(')&&index.includes('q=4')&&index.includes("out.filter='blur(4px)'")&&index.includes("setProperty('--frost'"),'frost blur is sampled at 1/4 resolution (4px blur = 16px full-scale) while the pattern DOM stays full-res');
test((index.match(/var\(--frost\)/g)||[]).length>=10&&index.includes('background-attachment:scroll, fixed'),'all translucent blocks sample the shared viewport-aligned frost snapshot');
test(!index.includes('backdrop-filter:blur(16px)'),'no live backdrop-filter remains: blur comes from the downsampled snapshot');
...
test(index.includes('.plan.selected { background-image:linear-gradient(90deg, rgba(222,221,116,.16)')&&index.includes('--tint-dark:rgba(20,20,20,.55)')&&index.includes('.plan { border:none; border-radius:36px; padding:16px 18px; cursor:pointer; background-image:linear-gradient(var(--tint-dark)'),'the selected plan is the colored card; unselected plans use the shared dark tint');
test(index.includes('input[type="email"], input[type="text"] { width:100%; height:44px; border:none; outline:none; color:#fff; font:inherit; font-size:14px; font-weight:500; padding:0 22px; border-radius:10000px; background-image:linear-gradient(var(--tint-dark)')&&index.includes('.pay:disabled { background:var(--tint-dark)'),'email, promo, and the disabled pay button share the inactive plan color');
