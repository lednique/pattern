...
test(index.includes('class="hero"')&&index.includes('.hero { margin-bottom:20px; background:var(--tint-dark)')&&index.includes('class="heroTop"')&&index.includes('.featureBar { display:grid; grid-template-columns:repeat(3,1fr)')&&!index.includes('.featureBar { display:flex'),'title, lead to the right, and horizontal features live in one dark block without a separate feature backdrop');
...
test(index.includes('rgba(5,7,8,0) 400px, rgba(5,7,8,.55) 600px'),'background dimming is fully transparent for 400px, then fades in over 200px');
