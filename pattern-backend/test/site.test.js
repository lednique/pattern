...
test(index.includes("decor=Math.random()<0.35?{emoji:pick(EMOJI)}")&&index.includes("shiftX===0&&shiftY===0"),'emoji appears only as the plugin allows: an intersection symbol when offsets are 0');
test(index.includes("fig1=Math.random()<0.18?{emoji:pick(EMOJI)}"),'an emoji can also be the regular pattern figure, never random sprinkling');
