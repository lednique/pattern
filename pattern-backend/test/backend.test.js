...
process.env.PRICE_ANNUAL='690';
process.env.PRICE_LIFETIME='1790';
...
(async function(){
  ...
  test(prices.data.annual===690&&prices.data.lifetime===1790&&prices.data.currency==='RUB','prices are exactly 690 ₽ and 1 790 ₽');
  ...
  test(annual.data.ok&&annual.data.form.OutSum==='690.00','annual Robokassa payment is 690.00 RUB');
  const lifetime=await call(create,JSON.stringify({plan:'lifetime',email:'life@example.com'}));
  test(lifetime.data.ok&&lifetime.data.form.OutSum==='1790.00','lifetime Robokassa payment is 1790.00 RUB');
  test(lifetime.data.form.Description.includes('бессрочная'),'payment description identifies lifetime plan');
  const signature=crypto.createHash('md5').update('test-shop:690.00:'+annual.data.form.InvId+':pass1').digest('hex');
  ...
  const inv=annual.data.form.InvId,out='690.00';
