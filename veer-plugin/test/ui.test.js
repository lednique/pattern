...
test(source.includes('.cols2{display:grid;grid-template-columns:1fr 1fr') && source.includes('id="startAngle"') && source.includes('id="bend"') && source.includes('id="overlap"'), 'fan parameters are laid out in two columns');
...
test(source.includes('id="startAngle" type="range" min="0" max="360"') && source.includes('id="bend" type="range" min="0" max="360"') && source.includes('id="overlap" type="range" min="0" max="90"'), 'angle and overlap ranges match the specification');
...
test(source.includes('grid-template-columns:repeat(5,90px)') && source.includes('VeerCore.TEMPLATES.forEach'), 'the preset overlay shows the 10 templates in a 5 × 2 grid');
...
test(source.includes('setPointerCapture') && source.includes('order.splice(view.dropTarget,0,moving)'), 'dragging a card reorders the stack');
